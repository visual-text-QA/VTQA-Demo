from config.path_cfgs import PATH
...
class Cfgs(PATH):
    def __init__(self):
        ...
        self.MAX_CONTEXT_TOKEN = 512
        self.MAX_QUESTION_TOKEN = 24
        self.TEST_AFTER_TRAIN = True
        ...
        # ALL LAYER: KECMR_NUM * (CMR_NUM_PER_KECMR + 1)
        self.KECMR_NUM = 2
        self.CMR_NUM_PER_KECMR = 2
        self.IMG_KEY_NUM = 32
        self.TEXT_KEY_NUM = 32
    ...
    def proc(self):
        assert self.RUN_MODE in ['train', 'val', 'test', 'test_dev']
        ...
        if self.CKPT_PATH is not None:
            ...
            self.CKPT_VERSION = (
                self.CKPT_PATH.split('/')[-1] + '_' + str(random.randint(0, 99999999))
            )
        ...
        self.EVAL_BATCH_SIZE = 1 if self.EVAL_BATCH_SIZE == 0 else self.EVAL_BATCH_SIZE
